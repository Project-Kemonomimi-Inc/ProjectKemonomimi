...
for ext in "${OUT_FORMATS[@]}"; do
  ...
  q="${!v:-$QUALITY_DEFAULT}"
  a_var="ARGS_${ext}"
  ARGS_FOR_CONVERT="${!a_var:-$ARGS_DEFAULT}"
  export ARGS_FOR_CONVERT
  ...
  if [[ "$CONV_IS_GM" -eq 1 ]]; then
    ...
    find . -type f -name "*.png" -print0 | sort -z | \
      parallel --bar --joblog "$log_file" -0 -j"${jobs}" '
        ...
        # mtime-aware skipping
        if [ -e "$out" ]; then
          if [ "$out" -nt "$f" ]; then
            echo "  ↳ exists & up-to-date, skipping"
            exit 0
          else
            echo "  ↳ exists but older than source, reconverting"
          fi
        fi

        if [ -n "$ARGS_FOR_CONVERT" ]; then
          # shellcheck disable=SC2206
          extra_args=($ARGS_FOR_CONVERT)
        else
          extra_args=()
        fi

        '"$CONV_BIN"' convert -verbose "$f" -strip "${extra_args[@]}" -quality '"$q"' "$out" \
          1>>"'"$log_file"'.verbose" 2>&1 &&
    ...
    find . -type f -name "*.png" -print0 | sort -z | \
      parallel --bar --joblog "$log_file" -0 -j"${jobs}" '
        ...
        '"$CONV_BIN"' -verbose "$f" -strip "${extra_args[@]}" -quality '"$q"' "$out" \
          1>>"'"$log_file"'.verbose" 2>&1 &&
        touch -r "$f" "$out" || echo "  ↳ warning: failed to copy mtime for $out"
      '
  fi
